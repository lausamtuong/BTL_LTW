...
const Payment = () => {
  ...
  const paymentHandle = () => {
    payment({
      ...
      amount:discount?discount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")+' vnđ': item[0].price,
      total:
        Number(items.total) +
        discount|| Number(item[0].price.split(" ")[0].replaceAll(".", "")),
    });
  };
  return (
    <form className="payment" onSubmit={submitHandle}>
      ...
      <div className="payment__customer">
          ...
          <div className="input">
            ...
            <button
              ...
              onClick={() => {
                ...
                alert("Áp dụng voucher giảm 10% thành công");
              }}
            >
              ÁP DỤNG
            </button>
        ...
        <div className="thanhtoan">
          ...
          <span>
            ...
            {discount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")||item[0].price} vnđ cho VietFast bằng cách lựa chọn một trong các
      ...
      <div className="payment__card">
        ...
        <div className="total_price">
          <span>Tổng</span>
          {discount?( <p style={{ fontWeight: "bold", }}>{discount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} vnđ</p>): <p style={{ fontWeight: "bold" }}>{item[0].price}</p>}
